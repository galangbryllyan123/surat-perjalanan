...
//nilai
$filenya = "kegiatan.php";
$judul = "[KEGIATAN & ANGGARAN]. Data Kegiatan";
$judulku = "$judul";
$judulx = $judul;
$s = nosql($_REQUEST['s']);
...
if ($s == "edit")
	...
	$qx = mysqli_query($koneksi, "SELECT * FROM m_kegiatan ".
						"WHERE kd = '$kdx'");
...
if (($_POST['btnSMP']) OR ($_POST['nama']))
	...
	if ((empty($kode)) OR (empty($nama)))
		...
		if (empty($s))
			{
			//cek
			$qcc = mysqli_query($koneksi, "SELECT * FROM m_kegiatan ".
									"WHERE kode = '$kode'");
			...
			if (!empty($tcc))
				...
				{
				mysqli_query($koneksi, "INSERT INTO m_kegiatan(kd, kode, nama, postdate) VALUES ".
								"('$x', '$kode', '$nama', '$today')");
		...
		if ($s == "edit")
			{
			mysqli_query($koneksi, "UPDATE m_kegiatan SET kode = '$kode', ".
							"nama = '$nama', ".
							"postdate = '$today' ".
							"WHERE kd = '$kd'");
...
if ($_POST['btnHPS'])
	...
	for ($i=1; $i<=$jml;$i++)
		...
		//del
		mysqli_query($koneksi, "DELETE FROM m_kegiatan ".
						"WHERE kd = '$kd'");
		}
...
echo '<div class="row">
	...
	<form action="'.$filenya.'" method="post" name="formx">';
		...
		$sqlcount = "SELECT * FROM m_kegiatan ".
						"ORDER BY nama ASC";
		$sqlresult = $sqlcount;
