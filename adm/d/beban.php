...
if ($s == "edit")
	...
	$qx = mysqli_query($koneksi, "SELECT * FROM m_beban ".
						"WHERE kd = '$kdx'");
...
if (($_POST['btnSMP']) OR ($_POST['nama']))
	...
	if ((empty($kode)) OR (empty($nama)))
		...
		if (empty($s))
			{
			//cek
			$qcc = mysqli_query($koneksi, "SELECT * FROM m_beban ".
									"WHERE kode = '$kode'");
			...
			if (!empty($tcc))
				...
				{
				mysqli_query($koneksi, "INSERT INTO m_beban(kd, kode, nama, postdate) VALUES ".
								"('$x', '$kode', '$nama', '$today')");
		...
		if ($s == "edit")
			{
			mysqli_query($koneksi, "UPDATE m_beban SET kode = '$kode', ".
							"nama = '$nama', ".
							"postdate = '$today' ".
							"WHERE kd = '$kd'");
...
if ($_POST['btnHPS'])
	...
	for ($i=1; $i<=$jml;$i++)
		...
		//del
		mysqli_query($koneksi, "DELETE FROM m_beban ".
						"WHERE kd = '$kd'");
		}
