...
if ($s == "edit")
	...
	$qx = mysqli_query($koneksi, "SELECT * FROM m_eselon ".
						"WHERE kd = '$kdx'");
	$rowx = mysqli_fetch_assoc($qx);
	$kode = balikin($rowx['kode']);
	$nama = balikin($rowx['nama']);
	$ket = balikin($rowx['ket']);
	}
...
if (($_POST['btnSMP']) OR ($_POST['nama']))
	...
	if (empty($s))
		{
		//cek
		$qcc = mysqli_query($koneksi, "SELECT * FROM m_eselon ".
								"WHERE nama = '$nama'");
		...
		if (!empty($tcc))
			...
			{
			mysqli_query($koneksi, "INSERT INTO m_eselon(kd, kode, nama, ket, postdate) VALUES ".
							"('$x', '$kode', '$nama', '$ket', '$today')");
	...
	if ($s == "edit")
		{
		mysqli_query($koneksi, "UPDATE m_eselon SET kode = '$kode', ".
						"ket = '$ket', ".
						"nama = '$nama', ".
						"postdate = '$today' ".
						"WHERE kd = '$kd'");
...
if ($_POST['btnHPS'])
	...
	for ($i=1; $i<=$jml;$i++)
		...
		//del
		mysqli_query($koneksi, "DELETE FROM m_eselon ".
						"WHERE kd = '$kd'");
		}
...
echo '<div class="row">
	<div class="col-md-3">
		...
		KODE : 
		<br>
		<input name="kode" type="text" size="15" value="'.$kode.'" class="btn btn-warning" required>
		</p>
		...
		KET : 
		<br>
		<input name="ket" type="text" size="20" value="'.$ket.'" class="btn btn-warning" required>
		</p>
