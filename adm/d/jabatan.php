...
if ($s == "edit")
	...
	$qx = mysqli_query($koneksi, "SELECT * FROM m_jabatan ".
						"WHERE kd = '$kdx'");
	$rowx = mysqli_fetch_assoc($qx);
	$kode = balikin($rowx['kode']);
	$nama = balikin($rowx['nama']);
	$jenis = balikin($rowx['jenis']);
	}
...
if (($_POST['btnSMP']) OR ($_POST['nama']))
	...
	//nek null
	if ((empty($kode)) OR (empty($nama)) OR (empty($jenis)))
		{
		...
		if (empty($s))
			{
			//cek
			$qcc = mysqli_query($koneksi, "SELECT * FROM m_jabatan ".
									"WHERE jenis = '$jenis' ".
									"AND kode = '$kode'");
			...
			if (!empty($tcc))
				...
				{
				mysqli_query($koneksi, "INSERT INTO m_jabatan(kd, kode, nama, jenis, postdate) VALUES ".
								"('$x', '$kode', '$nama', '$jenis', '$today')");
		...
		if ($s == "edit")
			{
			mysqli_query($koneksi, "UPDATE m_jabatan SET kode = '$kode', ".
							"nama = '$nama', ".
							"jenis = '$jenis', ".
							"postdate = '$today' ".
							"WHERE kd = '$kd'");
...
if ($_POST['btnHPS'])
	...
	for ($i=1; $i<=$jml;$i++)
		...
		//del
		mysqli_query($koneksi, "DELETE FROM m_jabatan ".
						"WHERE kd = '$kd'");
		}
...
echo '<div class="row">
	...
	<form action="'.$filenya.'" method="post" name="formx2">
		...
		JENIS : 
		<br>
		<input name="jenis" type="text" size="20" value="'.$jenis.'" class="btn btn-warning" required>
		</p>
