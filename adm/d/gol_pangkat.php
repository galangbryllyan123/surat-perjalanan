...
//nilai
$filenya = "gol_pangkat.php";
$judul = "[MASTER]. Data Golongan/Pangkat";
$judulku = "$judul";
$judulx = $judul;
$s = nosql($_REQUEST['s']);
...
if ($s == "edit")
	...
	$qx = mysqli_query($koneksi, "SELECT * FROM m_gol_pangkat ".
						"WHERE kd = '$kdx'");
	$rowx = mysqli_fetch_assoc($qx);
	$kode = balikin($rowx['kode']);
	$golongan = balikin($rowx['golongan']);
...
if (($_POST['btnSMP']) OR ($_POST['nama']))
	...
	if (empty($s))
		{
		//cek
		$qcc = mysqli_query($koneksi, "SELECT * FROM m_gol_pangkat ".
								"WHERE kode = '$kode'");
		...
		if (!empty($tcc))
			...
			{
			mysqli_query($koneksi, "INSERT INTO m_gol_pangkat(kd, kode, golongan, nama, postdate) VALUES ".
							"('$x', '$kode', '$golongan', '$nama', '$today')");
	...
	if ($s == "edit")
		{
		mysqli_query($koneksi, "UPDATE m_gol_pangkat SET kode = '$kode', ".
						"golongan = '$golongan', ".
						"nama = '$nama', ".
						"postdate = '$today' ".
						"WHERE kd = '$kd'");
...
if ($_POST['btnHPS'])
	...
	for ($i=1; $i<=$jml;$i++)
		...
		//del
		mysqli_query($koneksi, "DELETE FROM m_gol_pangkat ".
						"WHERE kd = '$kd'");
		}
...
echo '<div class="row">
	<div class="col-md-3">
		...
		GOLONGAN : 
		<br>
		<input name="golongan" type="text" size="15" value="'.$golongan.'" class="btn btn-warning" required>
		</p>
		...
		PANGKAT : 
		<br>
		<input name="nama" type="text" size="20" value="'.$nama.'" class="btn btn-warning" required>
		</p>
	...
	<form action="'.$filenya.'" method="post" name="formx">';
		...
		echo '<table id="example" class="table table-striped table-bordered row-border hover order-column" style="width:100%">
			...
			<td width="50"><strong><font color="'.$warnatext.'">GOLONGAN</font></strong></td>
			<td width="600"><strong><font color="'.$warnatext.'">PANGKAT</font></strong></td>
			</tr>
		...
		if ($count != 0)
			...
			do {
				...
				$golongan = balikin($data['golongan']);
