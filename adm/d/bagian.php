...
if ($s == "edit")
	...
	$qx = mysqli_query($koneksi, "SELECT * FROM m_bagian ".
						"WHERE kd = '$kdx'");
...
if (($_POST['btnSMP']) OR ($_POST['nama']))
	...
	if ((empty($kode)) OR (empty($nama)))
		...
		if (empty($s))
			{
			//cek
			$qcc = mysqli_query($koneksi, "SELECT * FROM m_bagian ".
									"WHERE kode = '$kode'");
			...
			if (!empty($tcc))
				...
				{
				//insert
				mysqli_query($koneksi, "INSERT INTO m_bagian(kd, kode, nama, postdate) VALUES ".
								"('$x', '$kode', '$nama', '$today')");
		...
		if ($s == "edit")
			{
			mysqli_query($koneksi, "UPDATE m_bagian SET kode = '$kode', ".
							"nama = '$nama', ".
							"postdate = '$today' ".
							"WHERE kd = '$kd'");
...
if ($_POST['btnHPS'])
	...
	for ($i=1; $i<=$jml;$i++)
		...
		//del
		mysqli_query($koneksi, "DELETE FROM m_bagian ".
						"WHERE kd = '$kd'");
		}
