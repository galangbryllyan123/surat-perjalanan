...
//nilai
$filenya = "jabatan_ttd.php";
$judul = "[MASTER]. Data Jabatan TTD";
$judulku = "$judul";
$judulx = $judul;
$s = nosql($_REQUEST['s']);
...
if (($_POST['btnSMP']) OR ($_POST['nama']))
	...
	if (empty($s))
		{
		//cek
		$qcc = mysqli_query($koneksi, "SELECT * FROM m_jabatan_ttd ".
								"WHERE kode = '$kode'");
		...
		if (!empty($tcc))
			...
			{
			mysqli_query($koneksi, "INSERT INTO m_jabatan_ttd(kd, kode, nama, postdate) VALUES ".
							"('$x', '$kode', '$nama', '$today')");
	...
	if ($s == "edit")
		{
		mysqli_query($koneksi, "UPDATE m_jabatan_ttd SET kode = '$kode', ".
						"nama = '$nama', ".
						"postdate = '$today' ".
						"WHERE kd = '$kd'");
...
if ($_POST['btnHPS'])
	...
	for ($i=1; $i<=$jml;$i++)
		...
		//del
		mysqli_query($koneksi, "DELETE FROM m_jabatan_ttd ".
						"WHERE kd = '$kd'");
		}
...
echo '<div class="row">
	...
	<form action="'.$filenya.'" method="post" name="formx">';
		...
		$sqlcount = "SELECT * FROM m_jabatan_ttd ".
						"ORDER BY nama ASC";
		$sqlresult = $sqlcount;
