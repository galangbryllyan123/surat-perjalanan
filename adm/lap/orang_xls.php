...
// judul
$sheet->setCellValue('A1', 'REKAP PER ORANG');
...
while ($row = mysqli_fetch_array($rs)) 
    ...
    $sheet->setCellValue('B'.$i, "$e_nama $i_gelar_belakang");
    $sheet->setCellValue('C'.$i, $i_gol_nama);
    $sheet->setCellValue('D'.$i, $i_jabatan_nama);
    $sheet->setCellValue('E'.$i, $i_spt_no);
    $sheet->setCellValue('F'.$i, $yuk2_tgl_dari);
    $sheet->setCellValue('G'.$i, $yuk2_tgl_sampai);
    $sheet->setCellValue('H'.$i, $yuk2_jml_lama);
    $sheet->setCellValue('I'.$i, "$yuk2_tujuan $yuk2_tujuan_1 $yuk2_tujuan_2");
...
// header Background color
    $excel->getActiveSheet()->getStyle('A'.$j.':J'.$j.'')->getFill()
    ->setFillType(\PhpOffice\PhpSpreadsheet\Style\Fill::FILL_SOLID)
    ->getStartColor()->setARGB('D3D3D3');
