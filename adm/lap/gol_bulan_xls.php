...
// judul
$sheet->setCellValue('A1', 'REKAP PER GOLONGAN & BULAN, TAHUN '.$tahunnya.'');
...
// header Background color
    $excel->getActiveSheet()->getStyle('A3:N3')->getFill()
...
for ($k=1;$k<=12;$k++)
	...
	$sheet->setCellValue(''.$kolomya.'3', ''.$blnya.'');
...
//bikin garis
$sqlku = mysqli_query($koneksi, "SELECT * FROM m_gol_pangkat ".
									"ORDER BY kode ASC");
...
while ($row = mysqli_fetch_array($rs)) 
  	...
  	$e_no = $e_no + 1;
	$i_kd = balikin($row['kd']);
	$i_nama = balikin($row['golongan']);
	$i_nama2 = cegah($row['golongan']);
	...
	for ($k=1;$k<=12;$k++)
		...
		$qyuk = mysqli_query($koneksi, "SELECT SUM(total_semuanya) AS total ".
											...
											"OR round(DATE_FORMAT(spt_tgl, '%m')) = '$kx') ".
...
// header Background color
    $excel->getActiveSheet()->getStyle('A'.$j.':N'.$j.'')->getFill()
    ->setFillType(\PhpOffice\PhpSpreadsheet\Style\Fill::FILL_SOLID)
    ->getStartColor()->setARGB('D3D3D3');
...
$ke = "rekap-per-gol-bulan-$tahunnya.xlsx";
