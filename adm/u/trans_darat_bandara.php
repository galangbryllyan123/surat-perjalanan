...
//nilai
$filenya = "trans_darat_bandara.php";
$judul = "[UANG & BIAYA]. Transportasi Darat dari Bandara";
$judulku = "$judul";
$judulx = $judul;
$s = nosql($_REQUEST['s']);
...
if ($s == "edit")
	...
	$qx = mysqli_query($koneksi, "SELECT * FROM m_tarif_transport_bandara ".
						"WHERE kd = '$kdx'");
...
if ($_POST['btnSMP'])
	...
	if (empty($s))
		{
		//cek
		$qcc = mysqli_query($koneksi, "SELECT * FROM m_tarif_transport_bandara ".
								"WHERE kode = '$e_kode'");
		...
		if (!empty($tcc))
			...
			{
			mysqli_query($koneksi, "INSERT INTO m_tarif_transport_bandara(kd, kode, nama, tarif, ket, postdate) VALUES ".
							"('$x', '$e_kode', '$e_nama', '$e_tarif', '$e_ket', '$today')");
	...
	if ($s == "edit")
		{
		mysqli_query($koneksi, "UPDATE m_tarif_transport_bandara SET kode = '$e_kode', ".
						"nama = '$e_nama', ".
						"tarif = '$e_tarif', ".
						"ket = '$e_ket', ".
						"postdate = '$today' ".
...
if ($_POST['btnHPS'])
	...
	for ($i=1; $i<=$jml;$i++)
		...
		//del
		mysqli_query($koneksi, "DELETE FROM m_tarif_transport_bandara ".
						"WHERE kd = '$kd'");
		}
...
echo '<div class="row">
	...
	<form action="'.$filenya.'" method="post" name="formx">';
		...
		$sqlcount = "SELECT * FROM m_tarif_transport_bandara ".
						"ORDER BY nama ASC";
		$sqlresult = $sqlcount;
